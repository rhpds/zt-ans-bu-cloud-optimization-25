...
d. Click **Next**, **Next**, then **Finish** to add the node.
--

. **Add the second node (Stop Instances).**
+
Hover over the `Retrieve INSTANCES by tag` node, click the **elipsis** icon, and select *Add step and link*. Configure it as follows:
+
--
a. **Node Type:** `Job Template`
b. **Job Template:** Select `Stop instances`.
c. **Status:** Select `Run On Success`.
d. Click **Next**, **Next**, then **Finish**.
--
